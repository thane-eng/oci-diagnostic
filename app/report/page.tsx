...
function ReportContent() {
  ...
  useEffect(() => {
    if (!engagementId) {
      setError('No engagement_id provided.')
      setLoading(false)
      return
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0f1f30] flex items-center justify-center">
        <p className="text-white/40 text-sm tracking-widest uppercase">Loading report…</p>
      </div>
    )
  }
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-[#0f1f30] flex items-center justify-center px-6">
        <div className="text-center">
          <p className="text-white/60 text-sm mb-2">Could not load report</p>
  ...
  // Sort elements lowest avg first (most attention needed)
  const sortedElements = [...ELEMENTS].sort(
    (a, b) => (data.elementAverages[a.key] ?? 0) - (data.elementAverages[b.key] ?? 0)
  )
    ...
    <div className="min-h-screen bg-[#0f1f30] text-white">
      ...
      <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between">
        <span className="text-white/40 text-sm font-medium tracking-widest uppercase">
          Bellomo Leadership · OCI Group Report
        </span>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="mb-10">
          <h1 className="text-3xl font-semibold text-white mb-1">
            {data.company || 'Group Report'}
          </h1>
          <p className="text-white/40 text-sm">
            Engagement ID: {data.engagementId} · {totalResponses} response{totalResponses !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        {dominantArchetype && archetypes[dominantArchetype] && (
          <div className="mb-10 bg-white/5 border border-white/15 rounded-2xl p-8">
            ...
            <h2 className="text-2xl font-semibold text-white mb-2">
              {archetypes[dominantArchetype].name}
            </h2>
            <span className="inline-block bg-white/10 text-white/60 text-xs font-semibold tracking-widest uppercase px-3 py-1 rounded-full mb-4">
              Primary gap: {archetypes[dominantArchetype].primaryGap}
            </span>
            <p className="text-white/70 text-base leading-relaxed italic">
              {archetypes[dominantArchetype].tagline}
            </p>
          </div>
        )}
        ...
        <div className="mb-10">
          ...
          <div className="bg-white/5 border border-white/15 rounded-2xl p-6 space-y-4">
              ...
              return (
                <div key={key}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-white/80">{arch.name.replace('The ', '')}</span>
                    <div className="flex items-center gap-3">
                      <span className="text-white/40 text-xs">{pct}%</span>
                      <span className="text-white/30 text-xs tabular-nums w-6 text-right">{count}</span>
                    </div>
                  </div>
                  <div className="bg-white/10 rounded-full h-2">
                    <div
                      className="bg-white/60 rounded-full h-2 transition-all duration-700"
                      style={{ width: `${barPct}%` }}
                    />
                  </div>
                </div>
            ...
            <p className="text-white/25 text-xs pt-1">
              Primary archetype per respondent. Percentages are of total responses.
            </p>
          </div>
        </div>
        ...
        <div className="mb-10">
          ...
          <div className="bg-white/5 border border-white/15 rounded-2xl p-6 space-y-5">
              ...
              return (
                <div key={el.key}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-white/80">{el.label}</span>
                    <div className="flex items-center gap-3">
                      <span className={`text-xs font-medium ${colors.text}`}>{colors.label}</span>
                      <span className="text-white/40 text-sm tabular-nums">{avg}/16</span>
                    </div>
                  </div>
                  <div className="bg-white/10 rounded-full h-2">
                    <div
            ...
            <p className="text-white/25 text-xs pt-2">
              Scores averaged across all respondents. Shown lowest to highest — lower scores indicate where courage gaps are creating the most drag.
            </p>
          </div>
        </div>
            ...
            <div className="bg-white/5 border border-white/15 rounded-2xl p-6">
              <div className="grid grid-cols-2 gap-3">
                {Object.entries(data.departmentCounts)
                  .sort((a, b) => b[1] - a[1])
                  .map(([dept, count]) => (
                    <div key={dept} className="flex items-center justify-between bg-white/5 rounded-lg px-4 py-3">
                      <span className="text-sm text-white/70">{dept}</span>
                      <span className="text-sm font-semibold text-white/50 tabular-nums">{count}</span>
                    </div>
                  ))}
              </div>
            </div>
            ...
            <div className="bg-white/5 border border-white/15 rounded-2xl p-6">
              <div className="grid grid-cols-2 gap-3">
                {Object.entries(data.roleCounts)
                  .sort((a, b) => b[1] - a[1])
                  .map(([role, count]) => (
                    <div key={role} className="flex items-center justify-between bg-white/5 rounded-lg px-4 py-3">
        ...
        <div className="bg-white rounded-2xl p-8 text-center">
          <p className="text-[#0f1f30] text-lg leading-relaxed mb-2 font-semibold">
            Ready to debrief these results?
          </p>
          <p className="text-[#0f1f30]/60 text-sm mb-6">
            Schedule a working session to translate this data into a concrete action plan.
          </p>
          <a
