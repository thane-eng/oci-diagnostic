...
function ReportContent() {
  ...
  if (loading) return <div className="min-h-screen bg-[#0f1f30] flex items-center justify-center"><p className="text-white/40 text-sm tracking-widest uppercase">Loading report...</p></div>
      if (error || !data) return <div className="min-h-screen bg-[#0f1f30] flex items-center justify-center px-6"><div className="text-center"><p className="text-white/60 text-sm mb-2">Could not load report</p><p className="text-white/30 text-xs">{error}</p></div></div>
              ...
              const dominantArchetype = Object.entries(data.archetypeCounts).sort((a, b) => b[1] - a[1])[0]?.[0] as ArchetypeKey | undefined
                  ...
                  return (
                        <div className="min-h-screen bg-[#0f1f30] text-white">
                              ...
                              <div className="max-w-3xl mx-auto px-6 py-12">
                                      <div className="mb-10">
                                                ...
                                                <p className="text-white/40 text-sm">Engagement ID: {data.engagementId} · {totalResponses} response{totalResponses !== 1 ? 's' : ''}</p>
                                      </div>
                                {dominantArchetype && archetypes[dominantArchetype] && (
                                    <div className="mb-10 bg-white/5 border border-white/15 rounded-2xl p-8">
                                                <p className="text-white/40 text-xs font-semibold tracking-widest uppercase mb-3">Dominant Pattern</p>
                                                ...
                                                <p className="text-white/70 text-base leading-relaxed italic">{archetypes[dominantArchetype].tagline}</p>
                                    </div>
                                      )}
                                      <div className="mb-10">
                                                <p className="text-white/40 text-xs font-semibold tracking-widest uppercase mb-4">Archetype Distribution</p>
                                                                                                                        ...
                                                                                                                        return (
                                                                                                                                          <div key={key}>
                                                                                                                                                            <div className="flex items-center justify-between mb-1">
                                                                                                                                                                                ...
                                                                                                                                                                                <div className="flex items-center gap-3"><span className="text-white/40 text-xs">{pct}%</span>span><span className="text-white/30 text-xs tabular-nums w-6 text-right">{count}</span>span></div>
                                                                                                                                                              </div>
                                                                                                                                                            <div className="bg-white/10 rounded-full h-2"><div className="bg-white/60 rounded-full h-2 transition-all duration-700" style={{ width: `${barPct}%` }} /></div>
                                                                                                                                            </div>
                                                                                                                                        )
                                                  })}
                                                            <p className="text-white/25 text-xs pt-1">Primary archetype per respondent.</p>
                                                </div>
                                      </div>
                                      <div className="mb-10">
                                                <p className="text-white/40 text-xs font-semibold tracking-widest uppercase mb-4">Element Scores — Group Average</p>
                                                                                        ...
                                                                                        return (
                                                                                                          <div key={el.key}>
                                                                                                                            <div className="flex items-center justify-between mb-2">
                                                                                                                                                ...
                                                                                                                                                <div className="flex items-center gap-3"><span className={`text-xs font-medium ${colors.text}`}>{colors.label}</span>span><span className="text-white/40 text-sm tabular-nums">{avg}/16</span>span></div>
                                                                                                                              </div>
                                                                                                                            <div className="bg-white/10 rounded-full h-2"><div className={`${colors.bar} rounded-full h-2 transition-all duration-700`} style={{ width: `${pct}%` }} /></div>
                                                                                                            </div>
                                                                                                        )
                                                  })}
                                                            <p className="text-white/25 text-xs pt-2">Averaged across all respondents. Lower = more drag.</p>
                                                </div>
                                      </div>
                                {Object.keys(data.departmentCounts).length > 0 && (
                                    <div className="mb-10">
                                                <p className="text-white/40 text-xs font-semibold tracking-widest uppercase mb-4">Responses by Department</p>
                                                <div className="bg-white/5 border border-white/15 rounded-2xl p-6"><div className="grid grid-cols-2 gap-3">{Object.entries(data.departmentCounts).sort((a, b) => b[1] - a[1]).map(([dept, count]) => (<div key={dept} className="flex items-center justify-between bg-white/5 rounded-lg px-4 py-3"><span className="text-sm text-white/70">{dept}</span>span><span className="text-sm font-semibold text-white/50 tabular-nums">{count}</span>span></div>))}</div></div>
                                    </div>
                                      )}
                                {Object.keys(data.roleCounts).length > 0 && (
                                    <div className="mb-10">
                                                <p className="text-white/40 text-xs font-semibold tracking-widest uppercase mb-4">Responses by Role Level</p>
                                      ...
                                      <div className="bg-white rounded-2xl p-8 text-center">
                                                <p className="text-[#0f1f30] text-lg leading-relaxed mb-2 font-semibold">Ready to debrief these results?</p>
                                                <p className="text-[#0f1f30]/60 text-sm mb-6">Schedule a working session to translate this data into a concrete action plan.</p>
                                      ...
                                      <div className="mt-12 text-center"><p className="text-white/20 text-xs">Bellomo Leadership · Operational Courage Index · {new Date().getFullYear()}</p></div>
                              </div>
                        </div>
                      )
                    }
                    ...
                    export default function ReportPage() {
                        return (
                              <Suspense fallback={<div className="min-h-screen bg-[#0f1f30] flex items-center justify-center"><p className="text-white/40 text-sm tracking-widest uppercase">Loading report...</p></div>}>
                                    <ReportContent />
                              </Suspense>Suspense>
                            )
                    }</div>
